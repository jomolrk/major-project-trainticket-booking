            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="page-header clearfix">
                        ...
                        <a href="ticketbook-create.php" class="btn btn-success float-right">Add New Record</a>
                    ...
                    $orderBy = array('seat_num', 'schedule_id', 'schedule', 'firstname', 'middlename', 'lastname', 'seat_type', 'fare_amount');
                    $order = 'id';
                    ...
                    $sql = "SELECT * FROM ticketbook ORDER BY $order $sort LIMIT $offset, $no_of_records_per_page";
                    ...
                    if(!empty($_GET['search'])) {
                        ...
                        $sql = "SELECT * FROM ticketbook
                            WHERE CONCAT_WS (seat_num,schedule_id,schedule,firstname,middlename,lastname,seat_type,fare_amount)
                            LIKE '%$search%'
                            ORDER BY $order $sort
                            ...
                            echo "<table class='table table-bordered table-striped'>";
                                echo "<thead>";
                                    echo "<tr>";
										...
										echo "<th><a href=?search=$search&sort=&order=schedule&sort=$sort>Date time</th>";
										...
										echo "<th><a href=?search=$search&sort=&order=seat_type&sort=$sort>Seat</th>";
										echo "<th><a href=?search=$search&sort=&order=fare_amount&sort=$sort>Amount</th>";
                                ...
                                while($row = mysqli_fetch_array($result)){
                                    echo "<tr>";
                                    echo "<td>" . $row['seat_num'] . "</td>";echo "<td>" . $row['schedule_id'] . "</td>";echo "<td>" . $row['schedule'] . "</td>";echo "<td>" . $row['firstname'] . "</td>";echo "<td>" . $row['middlename'] . "</td>";echo "<td>" . $row['lastname'] . "</td>";echo "<td>" . $row['seat_type'] . "</td>";echo "<td>" . $row['fare_amount'] . "</td>";
                                        echo "<td>";
									
                                            //echo "<a href='ticketbook-read.php?id=". $row['id'] ."' title='View Record' data-toggle='tooltip'><i class='far fa-eye'></i></a>";
                                            echo "<a href='ticketbook-update.php?id=". $row['id'] ."' title='Update Record' data-toggle='tooltip'><i class='far fa-edit'></i></a>";
