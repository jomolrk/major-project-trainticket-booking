...
if(isset($_GET['id'])){
    $trains = $conn->query("SELECT *,Concat(code,' - ',`name`) as train FROM `train` where id in (SELECT train_id FROM `schedule_list` where delete_flag = 0 and id in (SELECT schedule_id FROM `ticketbook` where id in ({$_GET['id']}) ) )");
    ...
    $qry = $conn->query("SELECT r.*,s.code as sched_code,s.train_id from `ticketbook` r inner join `schedule_list` s on r.schedule_id = s.id where r.id in ({$_GET['id']}) ");
    if($qry->num_rows > 0){
...
$train_group = ['','First Class','General'];
?>
<style>
