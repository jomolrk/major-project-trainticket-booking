        ...
        <div class="container-fluid">
			...
			<table class="table table-hover table-striped table-bordered">
				...
				<tbody>
					<?php 
						$i = 1;
						$trains = $conn->query("SELECT *,Concat(code,' - ',`name`) as train FROM `train` where id in (SELECT train_id FROM `schedule_list` where delete_flag = 0 and `type` = '{$type}')");
					...
					?>
						<tr>
							...
							<td class="px-0">
								...
								<div class="px-1"><span class="text-muted">General:</span> <span class="text-muted fa fa-user"></span> <b><?= isset($train_ef_arr[$row['train_id']]) ? $train_ef_arr[$row['train_id']] : 0 ?></b> <span class="text-muted ml-2 fa fa-tag"></span> <b><?= rtrim(number_format($row['economy_fare'],2),'.') ?></b></div>
							</td>
